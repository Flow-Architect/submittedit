...
function withoutExtensionSignature(event: LifecycleEventEnvelope): LifecycleEventEnvelope {
  return {
    ...
    ...(event.chainAnchor ? { chainAnchor: event.chainAnchor } : {}),
  };
}
...
function verificationState(timestamp: string): VerificationState {
  return { ...VERIFIED_EXTENSION_STATE, verifiedAt: timestamp };
}
...
export async function createOrUpdatePrivateReceiptBundle(
  ...
  const receipt = createReceipt({
    ...
    verification: verificationState(new Date().toISOString()),
  });
